...
module.exports = {
    ...
    run : async(client, message, args) => {
        ...
        if(!role) return message.channel.send('Il faut que tu spécifies un membre ! 🎄') //when no role is specified or pinged
        //now the code!
        await target.roles.remove(role) // removeing the role to the user
        message.channel.send(`${target.user.username}  j'ai retiré se rôle ! 🎄`) //this is optional and editable
    }
}
